...
var PublicKey = `
-----BEGIN CERTIFICATE-----
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCZoQy1fn5pEpCsLQ7CfXYCbJzoEVU3FFJsrBv0p5qzxG8Uk/ECelADTEoKm7crLI47E3X1EmXC/LXJm8vLbuhDeFTZFalB/NXDVbjhqoPlTvlk0w6vkyxp+tIR0WVN7JZZskx0MqDrpLpPpEL4ViFmUUNfZFCrIEx7V9sCzc9CTQIDAQAB
-----END CERTIFICATE-----
...
//公钥进行解密
func RSA_Decrypt(message string) (data []byte, err error) {
	var cipherText []byte
	cipherText, err = base64.StdEncoding.DecodeString(message)
	block, _ := pem.Decode([]byte(PublicKey))
	if block == nil {
		log.Println("decode certifacte key failed")
		err = errors.New("decode certifacte key failed")
		return
	}
	//pem解码
	var cert interface{}
	...
	if err != nil {
		log.Println("parse certifacte key failed, err=%s", err)
		return
	}
	data, err = PublicDecrypt(cert.(*rsa.PublicKey), cipherText)
	if err != nil {
		log.Println(err)
	}
